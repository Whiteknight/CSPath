...
namespace CSPath.Parsing.Parsers
{
    public class ThrowExceptionParser<TInput, TOutput> : IParser<TInput, TOutput>
    {
        ...
        public IParseResult<TOutput> Parse(ISequence<TInput> t) => throw new Exception(_getError(t));

        public IParseResult<object> ParseUntyped(ISequence<TInput> t) => throw new Exception(_getError(t));
    }
}